...
\newcommand{\mode}[1]{{\raisebox{-.15\height}{\includegraphics[width=0.4cm]{icons/mode_#1.png}}}}
...
As a tool (\mode{locusequation}), after selection it requires the two inputs described above.
...
\begin{enumerate}
    ...
    \begin{itemize}
    \item Alternatively, you can select the LocusEquation tool \mode{locusequation} and click $C'$ and then $C$ (or in a different order). This may be a simpler method for some users.
    \end{itemize}
...
representation of the computed algebraic equation.
    \begin{itemize}
    \item Alternatively, you can select the Envelope tool \mode{envelope} and click $f$ and then $C$ (or in a different order, this is also allowed).
    \end{itemize}
\end{enumerate}
...
investigations that are not supported by GeoGebra Discovery ART now. See \cite{BotanaRecio2017}
for some further details.
\item The graph of the implicit curve may be inaccurate in some cases. This can be improved by using the \texttt{Plot2D} command (see below).
\end{itemize}

\subsection{Discovering properties}

In this section we give a short overview on the Discover tool and command, and give an example of stepwise discovery.


\subsubsection{The Discover tool and command}

The Discover tool and command detect for geometric relationships between objects that are in true in general. They require an input point which is the target of investigation. Each property is checked first numerically, and then, if the result is positive, a second symbolic test is performed internally via the low level \texttt{Prove} command (see below).
...
\begin{enumerate}
    ...
    \end{center}
    \begin{itemize}
        \item Alternatively, select the \texttt{Discover} tool \mode{discover} and click point $C$.
    \end{itemize}
...
\setcounter{enumi}{3}
    ...
    \begin{itemize}
        \item Alternatively, you can click the small \mode{discover} icon in the Graphics View to enable the stepwise discovery mode.
    ...
    \begin{itemize}
        \item Alternatively, you can click the small \mode{discover} icon in the Graphics View to disable the stepwise discovery mode.
        GeoGebra Discovery will inform you on the current working mode in this case.
    \end{itemize}
\end{enumerate}
...
\paragraph{Example} Visualization of the tacnode curve $(x^2+y^2-3x)^2-4x^2(2-x)=0$ may be inaccurate if it is plotted directly. By using the \texttt{Plot2D} command, it is possible to analyze the curve via a CAD and plot it accurately. 
\begin{enumerate}
    \item Type the equation of the tacnode curve in the Input Bar. An equation labeled with $eq1$ is created.
    \item Type \texttt{Plot2D(eq1)} in the Input Bar.
    \begin{center}
    \includegraphics[scale=0.5]{tacnode1} \includegraphics[scale=0.5]{tacnode2}
    \end{center}
    \item To get the CADs accordingly to $x$ or $y$, enter \texttt{Plot2D(eq1,"x")} or \texttt{Plot2D(eq1,"y")} in the Input Bar.
    \begin{center}
    \includegraphics[scale=0.5]{tacnode3} \includegraphics[scale=0.5]{tacnode4}
    \end{center}
\end{enumerate}

Note: If GeoGebra's default plotting routine (without CAD) would skip plotting because the input cannot be handled properly, the \texttt{Plot2D} command jumps in automatically. For example, changing the equation sign to \texttt{>} in the first step, we automatically get the following output:
\begin{center}
\includegraphics[scale=0.5]{tacnode5}
\end{center}

\subsubsection{Comparison of segments in the Relation tool and command}

The Relation tool can compare lengths of segments symbolically. This may involve inequalities as well, and therefore it may require CAD computation. The command implicitly invokes the \texttt{Compare} low-level command (see below).

\paragraph{Example (Triangle Inequality)}
\begin{enumerate}
    \item Draw an arbitrary triangle $ABC$ with sides $a$, $b$ and $c$.
    \item Type \texttt{Relation(a+b,c)}. GeoGebra Discovery informs the user that ``$a+b$ and $c$ are not equal (checked numerically).''
